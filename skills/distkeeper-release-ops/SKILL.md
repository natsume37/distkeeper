...
将已经构建好的 APK、EXE、DEB 等制品归档到 distkeeper 配置的对象存储，并维护不可变版本和固定 latest 地址。该仓库是纯 CLI 工具；优先使用 `distkeeper --json ...`，不要新增或恢复 GUI/PySide6 代码。`status`、`plan_id` 和结构化 JSON 错误是 AI 调用的标准接口。
...
- `status` 是变更前的状态入口；不要从 `list` 的文本顺序猜测当前版本或回退目标。
- `plan` 与 `rollback --dry-run` 返回的 `plan_id` 绑定源文件 SHA-256、latest 版本和对象指纹；执行 `publish` 或 `rollback` 时应传入该 ID。
- 计划校验发生在执行前，不能替代分布式锁；同一 `repository + target + channel` 仍必须保持单发布者串行。
- OSS 凭据只能来自环境变量或受控的 `.env` 加载，不得打印、写入 YAML 或提交 Git。
- 任何真实 OSS 写入、覆盖 latest、回滚或远程清理，都必须在当前任务中获得明确授权；授权不等于可以扩大目标范围。

## 操作流程

1. 先执行 `status --json`，再检查输入文件、扩展名、版本格式、目标配置和 SHA-256，输出将要修改的逻辑键；先执行 `plan`，不要跳过预览。
...
4. 把 `plan_id` 传给 `publish` 或 `rollback` 执行；若使用显式版本保护，传入 `--expected-current-version`。计划或当前状态变化时停止并重新规划，不要强行重试。
5. 写入后立即执行 `verify`，再用 `list`/`tree` 检查清单和最终键。需要回退时先确认目标版本完整，再执行 `rollback` 并再次校验；发现历史归档路径错误时，按 [OSS runbook](references/oss-runbook.md) 的迁移流程处理。
6. 用 JSON 结果和进程退出码判断成功：`plan` 返回 `plan_id`、`scope_violations` 和 `requires_confirmation`；错误 JSON 包含 `code` 和 `retryable`，只重试可重试错误。完整性校验失败必须阻断后续自动发布。
7. 若计划显示越界，先停止并让用户确认；只有在明确确认后，才为变更命令添加 `--confirm-outside-scope`。不要把该参数作为默认自动化参数。
